...
const ddMM = function getCurrentDayDDMM() {
  let t = new Date()
  let dd = t.getDate()
  let ddOfWeek = t.getDay()
  let h = t.getHours()
  let mm = t.getMonth() + 1
...
gulp.task('replace', () => {
  return gulp
    ...
    .pipe(
      replace(
        /\[\[name\]\]|\[\[nama\]\]|\$Full name\$|\[\[نام\]\]|【姓名】|\[\[姓名\]\]|\[NAME\]/gi,
        '$Full Name$'
      )
    )
...
gulp.task('watch', () => {
  watch(
    [
      pathConfig.src.pug,
      pathConfig.src.pugTpl,
      pathConfig.db.dynamic,
      pathConfig.db.config
    ],
    () => {
      gulp.start('pug')
      // gulp.start('prod');
...
gulp.task('cleanGarbage', () => {
  return gulp.src('./src/*.html', { read: false }).pipe(rimraf({ force: true }))
})

gulp.task('cleanProd', () => {
  return gulp.src(prod, { read: false }).pipe(rimraf({ force: true }))
})
...
gulp.task('headers', () => {
  ...
  if (!fs.existsSync(`${__dirname}/build`) && dFiles.length > 0)
  ...
  dFiles.forEach((dFile, number, arr) => {
    ...
    let filePath = arr.length > 1
      ? `${__dirname}/build/headers-${number + 1}.html`
      : `${__dirname}/build/headers.html`
    fs.writeFileSync(filePath, file)
  })
})

gulp.task('init:params', () => {
  gulp
    .src(['src/config.js'], { base: './' })
    .pipe(replace('$taskName$', task))
    .pipe(replace(/(date: ')(....)(',)/g, `$1${ddMM()}$3`))
    .pipe(gulp.dest('./'))
})

gulp.task('default', cb => {
  runSequence('img:sync', 'init:params', 'style', 'pug', 'webserver', 'watch')
})
gulp.task('prod', cb => {
  runSequence(
    'cleanBeforeBuild',
